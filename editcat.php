<div class="wrap shopp">
	...
	<h2><?php _e('EDGE Category Editor','Shopp'); ?></h2>
	<div id="ajax-response"></div>
	<form name="category" id="category" action="<?php echo admin_url('admin.php'); ?>" method="post">
		<?php wp_nonce_field('shopp-save-edge-category'); ?>
		...
		<div id="poststuff" class="metabox-holder has-right-sidebar">
			...
			$side_meta_boxes = do_meta_boxes('shopp_page_shopp-importer-catmap', 'side', $EDGECategory);
			...
			<div id="post-body-content" class="has-sidebar-content">
				...
				<div id="titlediv">
					<div id="titlewrap">
						<input name="name" id="title" type="text" value="<?php echo esc_attr($EDGECategory->name); ?>" size="30" tabindex="1" autocomplete="off" />
					</div>
					<div class="inside">
						<?php if (SHOPP_PRETTYURLS && !empty($EDGECategory->id)): ?>
						<div id="edit-slug-box"><strong><?php _e('Permalink','Shopp'); ?>:</strong>
						<span id="sample-permalink"><?php echo $permalink; ?><span id="editable-slug" title="<?php _e('Click to edit this part of the permalink','Shopp'); ?>"><?php echo esc_attr($EDGECategory->slug); ?></span><span id="editable-slug-full"><?php echo esc_attr($EDGECategory->slug); ?></span>/</span>
...
var flashuploader = <?php echo ($uploader == 'flash' && !(false !== strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'mac') && apache_mod_loaded('mod_security')))?'true':'false'; ?>,
	category = <?php echo (!empty($EDGECategory->id))?$EDGECategory->id:'false'; ?>,
	product = false,
	details = <?php echo json_encode($EDGECategory->specs) ?>,
	priceranges = <?php echo json_encode($EDGECategory->priceranges) ?>,
	options = <?php echo json_encode($EDGECategory->options) ?>,
	prices = <?php echo json_encode($EDGECategory->prices) ?>,
	...
	adminpage = '<?php echo $this->Admin->pagename('importer-catmap'); ?>',
