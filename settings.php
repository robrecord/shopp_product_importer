...
<?php
 ...
 ?>
	...
	<div id="main-container">
		...
		<div id="error-show" style="-moz-border-radius:8px;-webkit-border-radius:8px;-moz-box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; border:1px solid #bbb;font-size:12px; font-weight:bold; padding:3px; margin:2px; background:#eee; color:#000; clear:both; padding:10px; display:none"></div>
		<?php if (!isset($importing_now) && ($this->Shopp->Settings->get('catskin_importer_file') != 'no-file-selected') && strlen($this->Shopp->Settings->get('catskin_importer_file')) > 0): ?>
			...
			<?php
				function doselector($column_number,$object_context) {
					...
					$output .= "<option value='edge_inventory_status' ".('edge_inventory_status'==$catskin_importer_type?'selected="selected"':'').">EDGE Inventory Status</option>";
					$output .= "<option value='edge_category_id' ".('edge_category_id'==$catskin_importer_type?'selected="selected"':'').">EDGE Category ID</option>";
					$output .= "<option value='edge_category_name' ".('edge_category_name'==$catskin_importer_type?'selected="selected"':'').">EDGE Category Name [Text]</option>";
					$output .= "<option value='edge_category_type' ".('edge_category_type'==$catskin_importer_type?'selected="selected"':'').">EDGE Category Type [Text]</option>";
					$output .= "<option value='edge_category_desc' ".('edge_category_desc'==$catskin_importer_type?'selected="selected"':'').">EDGE Category Description [Text]</option>";
				...
				if ($this->get_examine_data()) :
					// $this->examine_data = $spi_files->load_examine_csv($filename,true);
					// unset($spi_files);
						?>
						<p>Number of rows: <?= $_SESSION['spi_files_row_count'] ?></p>
						<table cellspacing='0' style='border:1px solid #999; width:100%;-moz-border-radius:8px;-webkit-border-radius:8px;-moz-box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; border:1px solid #bbb;font-size:12px; font-weight:bold; padding:3px; margin:2px; background:#eee; color:#000; clear:both; padding:10px; line-height:100%;'><tr><td style='border-bottom:1px solid #999; color:#fff; background:#BBB; padding:5px;'>Cols</td><td style='border-bottom:1px solid #999; color:#fff; background:#BBB;  padding:5px;'>First Row</td><td style='border-bottom:1px solid #999; color:#fff; background:#BBB;  padding:5px;'>Column Mapping</td><td style='border-bottom:1px solid #999; color:#fff; background:#BBB;  padding:5px;'>Second Row</td></tr>
						<? for ($key=0; $key < count($_SESSION['spi_files_header_row']); $key++) : ?>

					<tr><td style='border-bottom:1px solid #ccc; padding:5px; background:#eee;'><span style='color:#999;'>Col. <?= $key ?>:</span></td><td style='border-bottom:1px solid #ccc; padding:5px; background:#fff;'><b><?= $_SESSION['spi_files_header_row'][$key] ?></b></td><td style='border-bottom:1px solid #ccc; padding:5px; background:#eee;'><?= doselector($key,$this) ?></td><td style='border-bottom:1px solid #ccc; padding:5px; background:#fff;'><span style='color:#999;'>Next Row Data :</span> <?= $_SESSION['spi_files_first_row'][$key] ?></td></tr>
				...
				function next_image() {
					...
					jQuery.ajax({
						...
						success: function(response){
							...
							if (!response.match(/no-more/g)  && !response.match(/no-images/g)) {
								$("#imported-rows").append(response);
								next_image();
							} else {
								if (response.match(/no-more/g)) $("#imported-rows").append("<div style='clear:both;'></div><p>There are no more images to upload from this CSV file</p><p><b>Congratulations! Your CSV has been imported, images and all...</b></p>");
								if (response.match(/no-images/g)) $("#imported-rows").append("<div style='clear:both;'></div><p>There are no images to upload from this CSV file</p><p><b>Congratulations! Your CSV has been imported...</b></p>");
...
<script type="text/javascript">
	function validate() {
		...
		if (path_exists === "NO"){
			jQuery("option[value='description']").each(function() {
				if ((jQuery(this).attr("selected") == "selected") || (jQuery(this).attr("selected") == true)) {
					has_html_files = true;
				}
			});
			if (has_html_files) {
				validates = false;
				jQuery("#todo-list").append("<li>You need to specify the location of your uploaded HTML product description files, the one listed in 'Path to HTML files' doesn't exist.</li>");
			}
		}
		...
		*/
		...
		.add("option[value='edge_category_id']")
		.add("option[value='edge_category_name']")
		.add("option[value='edge_category_type']")
		.add("option[value='edge_category_desc']")
		.each(function() {
			if ((jQuery(this).attr("selected") == "selected") || (jQuery(this).attr("selected") == true)) {
				has_category = true;
			}
	...
	(function($){
		$('.field-type-selector').each(function() {
			if ($(this).attr("value") == 'variation' || $(this).attr("value") == 'spec') {
