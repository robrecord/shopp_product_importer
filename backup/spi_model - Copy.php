...
class spi_model {
						...
						foreach ($this->map as $mset)
						{
							if ($mset['type']=='image')
							{
								...
								if (!empty($filename))
								{
									$error = false;
									$report = true;
									$url_template = $this->Shopp->Settings->get('catskin_importer_imageformat');     
	...
	function execute_mega_query() {
		...
		if (isset($update_products)) {
			foreach ($update_products as $id => $product) {
				...
				if ($wpdb->query($query)) $this->spi->result['products_updated'][] = $this->products[$id]->sku;
			}
		}
	...
	function filter_by_image_presence($csv_product_id)
	{
		...
		if ( $this->Shopp->Settings
			...
			&& !$this->image_exists($csv_product_id) )
